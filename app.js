...
function updateUI() {
  ...
  var forecastDescriptionDay1 = document.querySelector(
    '.forecast-day-1 .conditions'
  );
  var forecastDescriptionDay2 = document.querySelector(
    '.forecast-day-2 .conditions'
  );
  var forecastDescriptionDay3 = document.querySelector(
    '.forecast-day-3 .conditions'
  );
  ...
  var forecastTempDay1 = document.querySelector(
    '.forecast-day-1 .forecast-num p'
  );
  var forecastTempDay2 = document.querySelector(
    '.forecast-day-2 .forecast-num p'
  );
  var forecastTempDay3 = document.querySelector(
    '.forecast-day-3 .forecast-num p'
  );
  ...
  //Slices string starting at index 1 and captures all chars except index 0 (first letter of string)
  // concatenates string
  var currentDescription =
    currentDescriptionData[0].toUpperCase() + currentDescriptionData.slice(1);
  ...
  sunsetData = weatherDataObj.current.sunset;
  ...
  forecastTempDay1.textContent =
    forecastTempDataDay1High + '°' + '/' + '°' + forecastTempDataDay1Low;
  ...
  forecastTempDay2.textContent =
    forecastTempDataDay2High + '°' + '/' + '°' + forecastTempDataDay2Low;
  ...
  forecastTempDay3.textContent =
    forecastTempDataDay3High + '°' + '/' + '°' + forecastTempDataDay3Low;
...
function addCity(weatherData) {
  ...
  //          High/Low
  citiesData2.textContent =
    kelvinToFh(weatherData[0].main.temp_max) +
    '°' +
    '/' +
    kelvinToFh(weatherData[0].main.temp_min) +
    '°';
  ...
  // Determine if added city needs to be saved
  if (localStorage.getItem('cities') !== null) {
    // create new cities array to capture cities array from local storage
    var cities = [];

    // Parses JSON string from local and constructs object. Saves to cities array
...
function geoCode(event) {
  ...
  fetch(
    'https://trueway-geocoding.p.rapidapi.com/Geocode?language=en&country=US&address=' +
      queryInput,
    ...
    .then(function (data) {
      ...
      getCitiesData(
        cityGeoObj.results[0].location.lat,
        cityGeoObj.results[0].location.lng
      );
    })
    .catch(function (err) {
      console.log(err);
...
function setProgressBar(uvi, progressBar) {
  ...
  if (
    ...
    if (uvi > 0 && uvi <= 2) {
      ...
      document
        .querySelector('.uv-index-progress i')
